...
output:
        --json file for every map that include the calculated values for each cluster in the generated map.
           the file will be placed in datasetPath/CNNMODEL/mapProcessed/mapname.json
              ...
              Elimina del la lista de coordenadas el valor del punto que estoy analizando e instancia un nuevo clasificador, para buscar los más cercanos
        Emplea una ponderacion de distacia para los pesos que le asigne al los puntos.
           Introduce el nuevo valor para el cluster en la lista de coordenadas, por lo cual en la siguiente iteracion estara disponible
...
for map in mapList:
        if map.endswith(".json"):
                ...
                flName = map.replace(".json","_Mknn_"+str(k)+".json" )
